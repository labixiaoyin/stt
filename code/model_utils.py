# -*- coding: utf-8 -*-
"""
Created on Fri Jul 16 10:23:13 2021
...
def attention(Q, K, V):
    ...
    a = a_norm(Q, K) #(batch_size, seq_length, seq_length)
    
    return  torch.matmul(a,  V) #(batch_size, seq_length, dim)

class AttentionBlock(torch.nn.Module):
    def __init__(self,d_model, dk):
        super(AttentionBlock, self).__init__()
        self.value = Value(d_model, dk)
        self.key = Key(d_model, dk)
        self.query = Query(d_model, dk)
...
class MultiHeadAttentionBlock(torch.nn.Module):
    def __init__(self, d_model,dk, n_heads):
        super(MultiHeadAttentionBlock, self).__init__()
        self.heads = []
        
        for i in range(n_heads):
            self.heads.append(AttentionBlock(d_model, dk))
        
        self.heads = nn.ModuleList(self.heads)
        
        self.fc = nn.Linear(n_heads * dk, d_model, bias = False)
