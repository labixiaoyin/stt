...
class Transformer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #spiatal
        s_x=torch.transpose(x,1,2)
        s_x=self.dropout(self.LN2(s_x))
        e_s=self.encs_s[0](s_x)
        for enc_s in self.encs_s[1:]:
            e_s=enc_s(e_s)  #e_s:[batch,n,d_model]
        

        e=torch.cat((e_s,e_t),1) #e:[batch,len+input_size,d_model]
        e=torch.transpose(e,1,2) 
        e=self.LN3(e) 
        e=torch.transpose(e,1,2) #e:[batch,len,d_model]
